...
15. **Cross-domain event consumers import from the publishing domain's contracts only — never from its domain package.** A service consuming events from another bounded context imports event schemas and type enums from `@mma/contracts/{publishing-domain}`. It must never import from `@mma/{publishing-domain}-domain`. See the `cross-domain-event-handler` skill.
...
24. **ACL adapters must forward the originating `Authorization` header via `getOutboundHeaders()` from `@mma/telemetry`.** This sends BOTH `x-correlation-id` AND `Authorization: Bearer ...` so the downstream `@CurrentUser()` resolves to the same actor end-to-end. Use `getOutboundHeaders()` (full) instead of the legacy `getCorrelationHeaders()` (correlationId-only) for any cross-service call to an authenticated endpoint.
...
2. **Respect Clean Architecture boundaries** and the Golden Rules above; use `@mma/contracts/{domain}` subpath imports for DTOs/schemas.

...
describe('DeliverOrderUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new DeliverOrderUseCase(mockRepo);
  });

  it('should transition SHIPPED → DELIVERED', async () => {
    const order = makeOrder(OrderStatusEnum.SHIPPED);
    ...
    expect(result.getOrderStatus()).toBe(OrderStatusEnum.DELIVERED);
  ...
  it('should throw when order is not SHIPPED', async () => {
    mockRepo.findById.mockResolvedValue(makeOrder(OrderStatusEnum.PROCESSING));

...
describe('StartProcessingOrderUseCase', () => {
  let useCase: StartProcessingOrderUseCase;
  let mockRepo: jest.Mocked<IOrderRepository>;

  beforeEach(() => {
    jest.clearAllMocks();
    mockRepo = createMockRepository();
    useCase = new StartProcessingOrderUseCase(mockRepo);
  });
  ...
  it('should transition CONFIRMED → PROCESSING', async () => {
    ...
    expect(result.getOrderStatus()).toBe(OrderStatusEnum.PROCESSING);
  ...
  it('should throw when order is not CONFIRMED', async () => {

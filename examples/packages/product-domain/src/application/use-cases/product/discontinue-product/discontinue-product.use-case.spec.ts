import { DiscontinueProductUseCase } from './discontinue-product.use-case';
...
import { CannotDiscontinueProductError } from '../../../../domain/exceptions';
...
describe('DiscontinueProductUseCase', () => {
  let useCase: DiscontinueProductUseCase;
  let mockRepo: jest.Mocked<IProductRepository>;

  beforeEach(() => {
    jest.clearAllMocks();
    mockRepo = createMockProductRepository();
    useCase = new DiscontinueProductUseCase(mockRepo);
  });
  ...
  it('should discontinue an ACTIVE product', async () => {
    ...
    expect(result.getStatus()).toBe('DISCONTINUED');
  ...
  it('should throw CannotDiscontinueProductError when product is DELETED', async () => {

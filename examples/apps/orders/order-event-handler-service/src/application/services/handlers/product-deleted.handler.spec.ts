...
describe('ProductDeletedHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new ProductDeletedHandler(mockCancelDraftOrders as any);
  });

  it('should cancel draft orders for the deleted product', async () => {
    mockCancelDraftOrders.execute.mockResolvedValue({ cancelledOrderIds: ['ord-1'] });

    await handler.handle(
      { eventType: 'PRODUCT_DELETED', productId: 'prod-1', occurredAt: '2024-01-01T00:00:00.000Z' } as any,
      'msg-1',
    );

 ...
 * SqsLocalService
 *
 * Polls LocalStack SQS and delegates to OrderEventHandlerService.
...
export class SqsLocalService {
  ...
  constructor(
    private readonly orderEventHandlerService: OrderEventHandlerService,
  ) {}

  async pollQueue(): Promise<void> {
    const queueUrl = process.env.ORDER_EVENTS_SQS_QUEUE_URL;
    if (!queueUrl) {
      throw new Error('ORDER_EVENTS_SQS_QUEUE_URL is not defined');
    }
  ...
  private async processMessage(message: Message, queueUrl: string): Promise<void> {
    ...
    await this.orderEventHandlerService.handleRecords([normalized]);

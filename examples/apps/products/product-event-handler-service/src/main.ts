...
initTelemetry('product-event-handler-service');
...
if (process.env.STAGE === 'local') {
  setImmediate(async () => {
    ...
    Logger.log('Starting local SQS polling...', 'ProductEventHandlerService');
...
export const handler = async (
  ...
  const app = await bootstrap();
  const eventHandlerService = app.get(ProductEventHandlerService);

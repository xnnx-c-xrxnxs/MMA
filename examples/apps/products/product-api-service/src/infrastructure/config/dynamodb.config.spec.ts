...
describe('DynamoDBConfig', () => {
  ...
  describe('getClient', () => {
    ...
    it('creates an AWS client when not local', () => {
      delete process.env.STAGE;
      delete process.env.DYNAMODB_ENDPOINT;
  ...
  describe('getTable', () => {
    it('creates a table for a given name and schema', () => {
      ...
      const table = DynamoDBConfig.getTable('ProductsTable', schema);
      expect(createTable).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'ProductsTable', schema }),
      );
    ...
    it('returns the same table instance for the same name (singleton)', () => {
      ...
      const t1 = DynamoDBConfig.getTable('ProductsTable', schema);
      const t2 = DynamoDBConfig.getTable('ProductsTable', schema);
      expect(t1).toBe(t2);

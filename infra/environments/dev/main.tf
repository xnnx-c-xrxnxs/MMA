...
# Composes all modules using data from service-registry.json.
# No hardcoded service names — everything derived from the registry.
...
module "dynamodb" {
  ...
  enable_pitr = false # Dev data is ephemeral — no need for point-in-time recovery
  tags        = var.tags
}
...
module "rds" {
  ...
  skip_final_snapshot  = true  # dev is a disposable environment — no final snapshot needed
...
# One Secrets Manager secret per environment holding all sensitive configuration.
# Lambda functions read AWS_SECRETS_ARN at cold-start via SecretsConfig.resolve()
# which populates process.env from the secret JSON. No sensitive values are ever
# injected directly as Lambda environment variables.
...
# Runs prisma migrations, seed scripts, and custom tasks via Lambda invocation.
# No ECS cluster, no Docker image — just a ZIP uploaded to S3 by the CD workflow.
...
module "lambda_runner" {
  ...
  # VPC only needed when RDS is present (Prisma migrations must reach the DB)
...
# Wires Terraform module outputs to the env var names declared in the registry.
# resolved_infra_vars: all infra outputs (consumed by webapp and worker services)
# resolved_all_vars:   adds FE_BASE_URL from webapp URL (consumed by API services)
...
locals {
  resolved_infra_vars = merge(
    ...
    # SQS queue URLs (by URL env var pattern — services reference queue URLs, not names)
    { for q in local.registry.infrastructure.sqsQueues :
      replace(q.envVar, "_NAME", "_URL") => module.sqs[q.envVar].queue_url
    },
    # Project secret ARN — Lambda resolves all sensitive vars from this single secret at startup
    { "AWS_SECRETS_ARN" = aws_secretsmanager_secret.project.arn },
    # Cross-service API URLs (from shared api-gateway — path-based routing)
    { for svc in local.registry.apiServices :
      "API_${upper(svc.domain)}_URL" => "${module.api_gateway.api_endpoint}/${svc.domain}/api"
    },
    # Webapp API URLs (NEXT_PUBLIC_*)
    { for svc in local.registry.apiServices :
...
# ─── API Services (Lambda + API Gateway) ────────────────────────────────────
...
module "api_services" {
  ...
  # Gateway-side JWT auth (two-tier with NestJS JwtAuthGuard).
  # public_routes is filtered from .github/service-registry.json
  # → gatewayAuth.publicRoutes for THIS service. Counts must match the
  # service's @Public() decorators — enforced by gateway-public-routes-sync.
  ...
  environment_variables = merge(
    { STAGE = var.environment, NODE_ENV = "production", DOMAIN_PREFIX = each.value.domain, SWAGGER_ENABLED = "true" },
  ...
  dynamodb_table_arns = [
    for t in local.registry.infrastructure.dynamodbTables :
    module.dynamodb[t.domain].table_arn
    if t.domain == each.value.domain
  ]
...
# Provides a stable HTTPS URL, global edge caching of static assets, and a
# future attachment point for custom domains (ACM + Route 53).
...
# Downloads go through CloudFront signed URLs (long expiry, edge-cached).
# Uploads still use S3 presigned PUT URLs directly (no caching needed).

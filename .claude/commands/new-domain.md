---
description: "Scaffold a complete new domain from scratch — domain package, contracts, microservice, infrastructure, and use cases. Powered by the @old-st/nx-plugin:domain generator. USE WHEN user says 'add a domain', 'create a new domain', 'new bounded context', 'scaffold a domain', 'new entity from scratch', or describes a brand-new capability that doesn't map to an existing domain."
---
...
service composition) is delegated to **`@old-st/nx-plugin:domain`**. Your job is
to:
...
- [ ] **(M4)** Verify the application service declares a module-level singleton `const logger = createLogger('{domain}-api-service')` from `@old-st/telemetry` — this is mandatory and lint-checked (Golden Rule #35 / `app-service-has-logger`). Never use `new Logger()` from `@nestjs/common`.
...
Generated by:
  nx g @old-st/nx-plugin:domain --name={domain} --persistence={dynamo|prisma}
  nx g @old-st/nx-plugin:service --domain={domain}"

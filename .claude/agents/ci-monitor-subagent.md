...
This subagent uses Nx Cloud MCP tools for CI monitoring. The complete tool reference — including `nx_cloud_cipe_details`, `nx_cloud_fix_cipe_failure`, `update_self_healing_fix`, and `nx-cloud apply-locally` — is documented in the **MCP Tool Reference** section below.

No file-system writes, no `git` commits, no `Agent` calls. The orchestrator (`monitor-ci.md`) owns all mutations based on the status returned here.

...
export default {
    ...
    <ul>
      ...
      <li><strong>Domain Component</strong> — renders one slice of domain data using <code>@mma/ui</code> primitives and React Query hooks.</li>
      <li><strong>React Query Hook</strong> — owns server state (loading, error, data, refetch). Calls one API client method.</li>
      <li><strong>API Client</strong> — typed <code>fetch</code> wrapper. Parses every response with a Zod schema from <code>@mma/contracts/{domain}</code>.</li>
    ...
    <p>Shared contracts (<code>@mma/contracts/{domain}</code>) live outside this stack. They are imported at the boundary layers — by API clients (response validation) and by forms (Zod resolver) — and by the NestJS backend (<code>ZodValidationPipe</code>). One schema, two runtimes.</p>
  `,
...
export const LAYER_DEPENDENCIES: Record<FrontendLayer, FrontendLayer[]> = {
  ...
  ApiClient:       [],   // leaf — only imports @mma/contracts and fetch
...
// Rule 20:  All API calls go through @mma/client-common hooks.
// Rule 21:  UI primitives come from @mma/ui — never raw HTML elements.
...
// Rule 23n: Every @mma/ui primitive ships 4 files: tsx + index.ts + stories + spec.
// Rule 23o: Never use template-literal Tailwind class interpolation (JIT can't emit those classes).`,
    ...
    pitfalls: [
        ...
        '<strong>Typing API responses manually:</strong> hand-writing <code>type UserResponse = { ... }</code> instead of importing from <code>@mma/contracts/user</code> causes the types to drift the moment the backend changes.',

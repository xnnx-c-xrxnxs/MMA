...
const TABLE_CONFIGS: TableConfig[] = [
  ...
  {
    // ── CUSTOMER table (auto-generated by @mma/nx-plugin) ──
    // Driven by: CUSTOMERS_DYNAMODB_TABLE_NAME
    // Schemas contributing GSIs:
    //   • CustomerSchema (packages/customer-domain/src/infrastructure/schemas/CustomerSchema.ts)
    tableName: process.env.CUSTOMERS_DYNAMODB_TABLE_NAME || 'CUSTOMERS',
    description: 'customer-domain — CustomerSchema',
    primaryHashKey: 'PK',
    primarySortKey: 'SK',
    attributeDefinitions: [
      { name: 'PK', type: 'S' as const },
      { name: 'SK', type: 'S' as const },
      { name: 'GSI1PK', type: 'S' as const },
      { name: 'GSI1SK', type: 'S' as const },
      { name: 'GSI2PK', type: 'S' as const },
      { name: 'GSI2SK', type: 'S' as const },
      { name: 'GSI3PK', type: 'S' as const },
      { name: 'GSI4PK', type: 'S' as const },
    ],
    gsis: [
      { indexName: 'GSI1', hashKey: 'GSI1PK', sortKey: 'GSI1SK' }, // CustomerSchema: list customers by status sorted by dateCreated
      { indexName: 'GSI2', hashKey: 'GSI2PK', sortKey: 'GSI2SK' }, // CustomerSchema: list customers by tier sorted by dateCreated
      { indexName: 'GSI3', hashKey: 'GSI3PK' }, // CustomerSchema: resolve customer by userId (unique)
      { indexName: 'GSI4', hashKey: 'GSI4PK' }, // CustomerSchema: email uniqueness + lookup (unique)
    ],
  },
];
// ─────────────────────────────────────────────────────
...
function buildGlobalSecondaryIndexes(
  config: TableConfig,
...
async function createTable(config: TableConfig): Promise<void> {
  console.log(
    `  Creating table "${config.tableName}" (${config.gsis.length} GSIs)...`,
  );
  await client.send(
...
async function createQueue(config: QueueConfig): Promise<void> {
  let queueName =
    process.env[config.queueNameEnvVar] || config.queueNameDefault;
  ...
  console.log(
    `  Creating queue "${queueName}"${isFifo ? ' (FIFO)' : ' (Standard)'}...`,
  );
  ...
  const urlEnvVar = config.queueNameEnvVar.replace(
    /_QUEUE_NAME$/,
    '_QUEUE_URL',
  );
...
const S3_BUCKET_CONFIGS: S3BucketConfig[] = [
  {
    ...
    description:
      'file-api-service — stores uploaded files, served via presigned URLs',
    cors: true,
  },
];
...
async function createBucket(config: S3BucketConfig): Promise<void> {
  const bucketName =
    process.env[config.bucketNameEnvVar] || config.bucketNameDefault;
...
function deduplicateConfigs(configs: TableConfig[]): TableConfig[] {
  ...
  return configs.filter((c) => {
    if (seen.has(c.tableName)) {
      console.warn(
        `  [warn] Duplicate table name "${c.tableName}" — skipping second entry.`,
      );
      return false;
    }
...
async function main(): Promise<void> {
  ...
  if (QUEUE_CONFIGS.length > 0) {
    for (const queueConfig of QUEUE_CONFIGS) {
      const queueName =
        process.env[queueConfig.queueNameEnvVar] ||
        queueConfig.queueNameDefault;
      console.log(`Queue: ${queueName}`);
      console.log(`  ${queueConfig.description}`);

      const exists = existingQueueUrls.some((url) =>
  ...
  if (S3_BUCKET_CONFIGS.length > 0) {
    ...
    for (const bucketConfig of S3_BUCKET_CONFIGS) {
      const bucketName =
        process.env[bucketConfig.bucketNameEnvVar] ||
        bucketConfig.bucketNameDefault;

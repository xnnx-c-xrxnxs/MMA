...
## 10b. Webapp Deployment Mode

The webapp deploys as a **static Next.js export** (`output: 'export'`) to **S3 + CloudFront** for all environments. The mode is configured in `.github/service-registry.json`:
...
|---|---|---|---|
| `"static"` | all environments (default) | No ALB/ECS/Lambda cost. Global edge delivery via CloudFront. Zero cold starts. | No server-side rendering. No Edge middleware. Client-side auth guard only. |
| `"lambda"` | legacy / SSR experiments | Supports SSR and Edge middleware. | Lambda cold-start latency. Docker image required. |
| `"ecs"` | legacy / SSR production | Always-warm ALB target. Autoscaling. | ALB + Fargate baseline cost (~\$25/mo per environment). Docker image required. |

In **static mode** (the default), the CD workflow runs `pnpm nx build webapp` to produce `apps/webapp/out/`, then syncs the output to an S3 bucket and creates a CloudFront invalidation. There is no Docker build, no ECR push, and no ECS/Lambda resource.

The `infra/modules/static-webapp` module creates the S3 bucket (private, OAC-only access), CloudFront distribution with SPA routing (403/404 → `/index.html`), and a response headers security policy.

**Auth in static mode:** There is no `middleware.ts` (Edge middleware is incompatible with `output: 'export'`). Authentication redirect is client-side: `apps/webapp/src/app/(protected)/layout.tsx` calls `useAuth()` and redirects unauthenticated users to `/auth/login` via `useRouter`. The API re-validates every request regardless — the client-side guard is UX only.

**Switching to SSR mode** (rare — only if SSR is genuinely required):
1. Change `output` in `apps/webapp/next.config.js` from `'export'` to `'standalone'`.
2. Add a `Dockerfile` to `apps/webapp/`.
3. Change `deploymentMode` to `"lambda"` or `"ecs"` in `service-registry.json`.
4. Wire `module.webapp_lambda` or `module.webapp` in the environment root.

...
- Event schemas live in `@mma/contracts/{publishing-domain}` and include
...
| Cross-domain consumer imports `@mma/{other}-domain` | #15 | Grep the event-handler for `-domain` imports — only `@mma/contracts/{other}` is allowed. |
...
| Bare `@mma/contracts` import | #11 | Must be `@mma/contracts/{domain}`. |
...
5. Client hooks in `@mma/client-common` (`webapp-api-client-hooks`).
...
5. **Boundary grep:** confirm no cross-domain `-domain` imports and no bare `@mma/contracts`.
